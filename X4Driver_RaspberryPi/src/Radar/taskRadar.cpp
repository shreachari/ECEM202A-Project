...
int taskRadar(void)
{
...
#endif
    status = x4driver_set_iterations(x4driver, 30);
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
#ifdef DEBUG
...
#endif
    status = x4driver_set_pulses_per_step(x4driver, 300);
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
#ifdef DEBUG
    ...
    status = x4driver_set_frame_area_offset(x4driver, 0.18); // Given by module HW. Makes frame_area start = 0 at front of module.
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
#ifdef DEBUG
    ...
    status = x4driver_set_frame_area(x4driver, 0.3, 2.33); // Observe from 0.5m to 4.0m.
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
#ifdef DEBUG
    ...
    status = x4driver_set_tx_power(x4driver, TX_POWER_HIGH);
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
#ifdef DEBUG
        printf("Error in x4driver_set_frame_area\n");
    ...
    status = x4driver_set_fps(x4driver, 10); // Generate 5 frames per second
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
#ifdef DEBUG
